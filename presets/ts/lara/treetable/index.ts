  ...
  column: {
    headerCell: ({ context, props }: ColumnPassThroughMethodOptions) => ({
      class: [
        ...
        // Position
        { "sticky z-40": context.frozen },

        // Flex & Alignment
        "flex flex-1 items-center",
        {
          "flex-initial shrink-0": !context.frozen,
        },
        "text-left",
    ...
    bodyCell: ({ context }: ColumnPassThroughMethodOptions) => ({
      class: [
        // Position
        {
          sticky: context.frozen,
        },
    ...
    sortIcon: ({ context }: ColumnPassThroughMethodOptions) => ({
      class: [
        "ml-2 inline-block",
        context.sorted ? "fill-primary-700 dark:fill-white/80" : "fill-surface-700 dark:fill-white/70",
      ],
    }),
    sortBadge: {
      class: [
...
} satisfies TreeTablePassThroughOptions;
