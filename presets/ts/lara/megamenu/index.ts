...
export default {
  ...
  menuitem: ({ props }) => ({
    class: [
      "sm:relative static",
      {
        "sm:w-auto w-full": props.orientation === "horizontal",
        "w-full": props.orientation !== "horizontal",
      },
    ],
  }),
  content: ({ props, context, state }) => ({
    class: [
      // Shape
      { "rounded-md": state.focusedItemInfo.level < 1 && props.orientation === "horizontal" },

      //  Colors
      {
        "text-surface-500 dark:text-white/70": !context.focused && !context.active,
        "text-surface-500 dark:text-white/70 bg-surface-200 dark:bg-surface-600/90": context.focused && !context.active,
        "text-primary-700 dark:text-surface-0/80 bg-primary-50 dark:bg-primary-400/30": context.active,
      },
  ...
  submenuIcon: ({ props }) => ({
    class: [
      {
        "ml-auto sm:ml-2": props.orientation === "horizontal",
  ...
  panel: ({ props }) => ({
    class: [
      ...
      "z-10",
      {
        "sm:left-full top-0": props.orientation !== "horizontal",
      },
    ],
  }),
  grid: {
...
} satisfies MegaMenuPassThroughOptions;
