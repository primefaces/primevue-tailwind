<template>
    ...
    <div class="card">
        <MegaMenu :model="items" class="p-4 bg-surfrace-0 dark:bg-surface-900 shadow-md" style="border-radius: 3rem">
            ...
            <template #item="{ item }">
                <a v-if="item.root" v-ripple class="flex items-center cursor-pointer px-3 py-2 overflow-hidden relative font-semibold text-lg uppercase" style="border-radius: 2rem">
                    <span :class="item.icon" />
                    <span class="ml-2">{{ item.label }}</span>
                </a>
                <a v-else-if="!item.image" class="flex items-center p-3 cursor-pointer mb-2 gap-2">
                    <span class="inline-flex items-center justify-center rounded-full bg-primary-500 dark:bg-primary-400 w-[3rem] h-[3rem]">
